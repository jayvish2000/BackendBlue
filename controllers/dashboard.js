...
exports.getStatistics = (req, res, next) => {
    ...
    let countQuery = `SELECT 'user' AS name, COUNT(*) AS number FROM tbl_users UNION
                      SELECT 'product',      COUNT(*)           FROM tbl_products WHERE status = "1" UNION
                      SELECT 'booking',      COUNT(*)           FROM tbl_bookings WHERE booking_status != 'can' AND DATE(created_at) = CURDATE();`
    db.query(queryOrders, (err, result) => {
        ...
        if(completed_order.length){
            data.total_selling = completed_order.reduce((acc, item) => {return acc + parseInt(item.total) + parseInt(item.delivery_charge)}, 0);
        }
